...
class PageQuerySet(SearchableQuerySetMixin, TreeQuerySet):
    ...
    def unrestricted_q(self, cache_target=None):
    ...
    def not_view_restricted(self, cache_target=None):
        ...
        """
        return self.filter(self.unrestricted_q(cache_target))
    ...
    def view_restricted(self, cache_target=None):
        ...
        """
        return self.exclude(self.unrestricted_q(cache_target))

    def viewable_by_user_q(self, user, request=None):
        from wagtail.core.models import PageViewRestriction
        return self.unrestricted_q(request or user) | PageViewRestriction.get_permitted_objects_q(user, request)
    ...
    def public_q(self):
        warnings.warn(
            ...
            "restrictions (regardless of type), use PageQuerySet.unrestricted_q() instead.",
            category=RemovedInWagtail214Warning
        )
        return self.unrestricted_q()
