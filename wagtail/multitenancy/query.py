        ...
        matches = (
            self.annotate(
                match=Case(
                    ...
                    When(
                        hostname=hostname, is_default=True, then=MATCH_HOSTNAME_DEFAULT
                    ),
...
class TenantManager(models.Manager):
    def get_default(self, cache_target=None) -> "Tenant":
        cache_attr_name = "_default_wagtail_tenant"
        if cache_target and hasattr(cache_target, cache_attr_name):
            return getattr(cache_target, cache_attr_name)
        default_tenant = self.get(is_default=True)
        if cache_target:
            setattr(cache_target, cache_attr_name, default_tenant)
...
class TenantScopedQuerySet(models.QuerySet):
    ...
    def native_to(self, tenant: "Tenant") -> "TenantMemberQuerySet":
        return self.filter(self.native_to_tenant_q(tenant))

    def not_native_to(self, tenant: "Tenant") -> "TenantMemberQuerySet":
